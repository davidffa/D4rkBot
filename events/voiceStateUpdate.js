module.exports.run = (client, oldState, newState) => {
    ...
    if (client.music.players.get(oldState.guild.id) && voiceChannel === client.music.players.get(oldState.guild.id).voiceChannel) {
        if (voiceChannel.members.size === 1) {
            ...
            client.music.players.get(oldState.guild.id).textChannel.send(':warning: Pausei a música porque fiquei sozinho no canal de voz, se ninguem aparecer irei sair em 5 minutos.');
